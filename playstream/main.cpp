...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    //result = system->createStream("C:/Users/rina/Downloads/AllegroNonTroppo.mp3", FMOD_HARDWARE | FMOD_LOOP_NORMAL | FMOD_2D, 0, &sound);
	result = system->createStream("C:/Users/rina/Downloads/flowers-midi.mid", FMOD_HARDWARE | FMOD_LOOP_NORMAL | FMOD_2D, 0, &sound);
	ERRCHECK(result);
	...
	printf("Press 's'/'S' to change channel speed (tempo)\n");
	printf("\n");
    ...
    do
    {
        if (_kbhit())
        {
            ...
            switch (key)
            {
				...
				// this is where i started to do stuff
				case 'v' :
                {
                    float volume;
				...
				case 's' :
				{
					...
					float thesound;
					sound->getMusicSpeed(&thesound);
					sound->setMusicSpeed(thesound-0.1f);
